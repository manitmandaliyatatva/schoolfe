...
export interface AddTeacherDocumentDialogData {
  documentTypes: ITextValueOption[];
}

export interface AddTeacherDocumentDialogResult {
...
@Component({
  selector: 'app-add-teacher-document-dialog',
  standalone: true,
  imports: [MatDialogModule, ReactiveFormsModule, CommonDropdownComponent, ButtonComponent, DocumentUploadComponent],
  templateUrl: './add-teacher-document-dialog.html',
  styleUrl: './add-teacher-document-dialog.scss',
})
export class AddTeacherDocumentDialog {
  private readonly fb = inject(FormBuilder);
  private readonly dialogRef = inject(MatDialogRef<any, AddTeacherDocumentDialogResult | null>);
  protected readonly data = inject<AddTeacherDocumentDialogData>(MAT_DIALOG_DATA);
